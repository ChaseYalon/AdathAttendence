...
// Generate detailed absence report for a specific class or all classes
window.generateAbsenceReport = async function() {
    const classId = document.getElementById('classSelect').value;
    const classes = await fetchClassesData();
    
    // If "All Classes" is selected, use the full list
    const selectedClasses = classId ? classes.filter(c => c.id === classId) : classes;
    ...
    selectedClasses.forEach(selectedClass => {
        ...
        selectedClass.students.forEach(student => {
            ...
            var absenceRate = ((absences / stats.totalSessions) * 100).toFixed(1);
            if(isNaN(absenceRate)){
                absenceRate=0
            }
